...
def step_KT2000_FD2_RFA(h, B_n, te, ti, q_n, abs_U_s, t_n, I):
    ...
    # --------------------------------------------------------------------------
    # Freezout:
    topo.freeze(phi_S, cryst_core, newtonian_efficiency, h, B_n, dBdt, ti, t_n)
    #
    # --------------------------------------------------------------------------
    # Active subsetting
    K_n = ne.evaluate('R_n * h**3') # much faster cubing
    I_ij = I[2:-2,2:-2] # (rows-4, cols-4)
    q_ij = q_n[2:-2,2:-2][I_ij] # (n_active,)
    h_ij = h[2:-2,2:-2][I_ij] # (n_active,)
    te_ij = te[2:-2,2:-2][I_ij] # (n_active,)
    B_ij = B_n[2:-2,2:-2][I_ij] # (n_active,)
    K_ij = K_n[2:-2,2:-2][I_ij] # (n_active,)
    R_ij = R_n[2:-2,2:-2][I_ij] # (n_active,)
    Rsurf_ij = Rsurf_n[2:-2,2:-2][I_ij] # (n_active,)
    ...
    #
    # --------------------------------------------------------------------------
    # Rheo Factors at edges & Diffusivities
    ...
    #
    # --------------------------------------------------------------------------
    # MUSCL reconstruction of edge states (linear)
    # t_erupted:
    all_te_states = np.array(muscl_linear(te, I,flux_lim_superbee)) # (8, n_active)
    te_RRUU = all_te_states[0::2] # (4, n_active)
    te_LLDD = all_te_states[1::2] # (4, n_active)
    ...
    # --------------------------------------------------------------------------
    # Surface Velocities:
    neg_Rsurf_grad_S = Rsurf * (neg_grad_B - grad_h)
    Usurf = neg_Rsurf_grad_S * 0.5 * (h2_RRUU + h2_LLDD) # (4, n_active) # order: E, W, N, S
    asurf = abs(neg_Rsurf_grad_S) * max_h2 # (4, n_active) # order: E, W, N, S
    #
    # --------------------------------------------------------------------------
    ...
    #
    # --------------------------------------------------------------------------
    #neg_advection_te = te_ij*np.sum(Usurf_EWNS/dxy[:,None], axis = 0) - np.sum(all_fluxes/dxy[:,None], axis = 0)
    neg_advection_te = np.sum((te_ij[None,:]*Usurf - all_fluxes_te) / dxy[:,None], axis = 0)
    RHS_h = q_ij - np.sum(all_fluxes_h / dxy[:,None], axis = 0)
    # --------------------------------------------------------------------------
    ...
    abs_Usurf = np.zeros(h.shape, dtype = h.dtype); abs_Usurf[I] = np.max(abs(Usurf), axis = 0)
    #
    # --------------------------------------------------------------------------
    substeps = 1
